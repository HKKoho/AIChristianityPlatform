...
const DeepDiveContent: React.FC<DeepDiveContentProps> = ({ deepDive, onBack }) => {
  ...
  // Auto-scroll to AI answer when it appears
  useEffect(() => {
    if ((aiAnswer || error) && aiAnswerRef.current) {
      setTimeout(() => {
  ...
  const handleGenerateAnswer = async () => {
    ...
    try {
      if (selectedModel === 'gemini') {
        ...
        const languageInstruction = i18n.language === 'zh-TW'
        ...
        setAiAnswer(t('bibleGame:ui.openAIPlaceholder'));
      }
    } catch (err) {
      console.error("Error generating AI answer:", err);
      setError(t('bibleGame:ui.aiError'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex-grow overflow-y-auto pr-4 -mr-4">
        <h2 id="deepdive-title" className="text-4xl font-bold text-amber-900 mb-4 flex items-center gap-3" style={{fontFamily: "'Trajan Pro', serif"}}>
            ...
            {t('bibleGame:ui.deepDiveTitle', { title: deepDive.title })}
        </h2>
        ...
        <h3 className="text-2xl font-bold text-amber-900 pb-2 mb-4 flex items-center gap-3">
            ...
            {t('bibleGame:ui.exploreDeeper')}
        </h3>
        <p className="text-stone-700 mb-4">{t('bibleGame:ui.exploreQuestion')}</p>

        <div className="mb-2">
          <span className="text-sm font-bold text-stone-600 mr-4">{t('bibleGame:ui.selectModel')}</span>
        ...
        <div className="flex gap-2">
          <textarea
              ...
              placeholder={t('bibleGame:ui.questionPlaceholder')}
        ...
        {error && <p ref={aiAnswerRef} className="mt-4 text-red-600 bg-red-100 p-3 rounded-lg">{error}</p>}

        {aiAnswer && (
            <div ref={aiAnswerRef} className="mt-4 p-4 bg-amber-50/70 border-l-4 border-amber-800 rounded-r-lg animate-fade-in">
        ...
        <div className="mt-8 border-t-2 border-amber-800/20 pt-6">
            ...
            >
              {t('bibleGame:ui.backToExplanation')}
            </button>
        </div>
    </div>
  );
